...
- On load, the app fetches Bitcoin's full daily closing-price history in INR
  and USD (used as a 1:1 proxy for USDT) from the [CoinGecko](https://www.coingecko.com)
  public API, and caches it in `localStorage` for an hour to avoid refetching.
...
- The CoinGecko public API is rate-limited; if you hit a rate limit, wait a
  moment and retry.
